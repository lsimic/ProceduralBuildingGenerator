...
def generate_vertical_layout(params, h_layout_verts):
    ...
    while i < len(h_layout_verts):
        ...
        # calculate window distance on x and y axis, calculate rotation.
        ...
        rot = vec_edge.xy.angle_signed(vec_0.xy)

        j = 0
        while j < window_count:
            # calculate window positions for this edge
            co_x = (v1[0] + ((dist_x - (window_count - 1) * ww_dist_x) / 2) + j * ww_dist_x)
            co_y = (v1[1] + ((dist_y - (window_count - 1) * ww_dist_y) / 2) + j * ww_dist_y)
            window_positions.append((co_x, co_y, rot))

            # calculate the last vert of this loop, because it is broken by the window
            v_x = co_x - 0.5 * window_width_x
            v_y = co_y - 0.5 * window_width_y
            # push it into the loops array
            if is_first_loop:
                wall_layout_verts_initial.append((v_x, v_y, 0))
                is_first_loop = False
            else:
                wall_layout_verts.append((v_x, v_y, 0))
        ...
        # TODO: calculate pillar positions
        i += 1
    # end for
